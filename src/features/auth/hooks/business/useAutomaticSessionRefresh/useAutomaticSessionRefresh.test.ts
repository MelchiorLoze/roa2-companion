...
const defaultGetEntityTokenValue: ReturnType<typeof useGetEntityToken> = {
  renew: renewMock,
  isLoading: false,
  isError: false,
};
...
describe('useAutomaticSessionRefresh hook', () => {
  ...
  it('calls renew when app state is active', () => {
    ...
    expect(defaultSessionValue.setSession).toHaveBeenCalledWith(newSession);
  });
  ...
  it('does not call renew when app state is inactive', () => {
    ...
    expect(defaultSessionValue.setSession).not.toHaveBeenCalled();
  });
  ...
  it('allows renewal after throttle period expires', () => {
    ...
    expect(renewMock).toHaveBeenCalledTimes(1);
    expect(defaultSessionValue.setSession).toHaveBeenCalledWith(otherSession);
    jest.useRealTimers();
  });
});
